...
class DEGA(object):
    def __init__(self, func, max_iter, dim, lb, ub, r, N_lst, initX, v_dv=None):
    ...
    def DEGAoperator(self):
        ...
        cmXd2 = np.zeros((self.popsize, self.o))
        encodelengths = []
        ...
        for i in range(self.popsize):
            for j in range(self.r, self.dim):
                v_ca = self.v_dv[j - self.r]
                for j2 in range(len(v_ca)):
                    if self.pop.X[i,j] == v_ca[j2]:
                       self.pop.X[i,j] = j2
        for i in range(m):
            chrome = []
            for j in range(self.r, self.dim):
        ...
        for i in range(self.popsize):
            for j in range(self.r, self.dim):
                ...
                for j2 in range(len(v_ca)):
                    ...
                    if self.pop.X[i,j] == j2:
                       self.pop.X[i,j] = v_ca[j2]
        cmX1 = np.hstack([cmXr, cmXd2])
